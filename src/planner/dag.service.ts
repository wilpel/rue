...
export class TaskDAG {
  ...
  getReadyNodes(): TaskNode[] {
    ...
    for (const [id, node] of this.nodes) {
      ...
      const allDone = Array.from(deps).every(depId => this.nodes.get(depId)?.status === "completed");
  ...
  topologicalOrder(): string[] {
    ...
    while (queue.length > 0) {
      ...
      order.push(current);
      for (const target of (this.edges.get(current) ?? new Set<string>())) {
        const nd = (inDegree.get(target) ?? 1) - 1;
        inDegree.set(target, nd);
        if (nd === 0) queue.push(target);
      }
    }
    return order;
  }
