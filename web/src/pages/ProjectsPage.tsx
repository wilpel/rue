...
export function ProjectsPage() {
  ...
  return (
    <div className="h-screen flex flex-col">
      <div className="border-b border-border-subtle bg-surface-1/50 px-6 py-3.5 flex items-center justify-between">
        <h1 className="text-sm font-semibold text-text-secondary tracking-wide">Projects</h1>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {loading ? (
          <div className="text-center py-24 animate-fade-in">
            <div className="w-2 h-2 rounded-full bg-gold/40 mx-auto animate-pulse-gold" />
          </div>
        ) : projects.length === 0 ? (
          <div className="text-center py-24 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl">
            {projects.map((p, i) => (
              <Link
                key={p.name}
                to={`/projects/${p.name}`}
                className={`group bg-surface-1 rounded-xl border border-border-subtle p-5 hover:border-gold/20 hover:bg-surface-2 transition-all duration-300 animate-fade-in stagger-${Math.min(i + 1, 5)}`}
              >
                <div className="flex items-start justify-between mb-3">
                  <h3 className="font-semibold text-text text-sm">{p.name}</h3>
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                    p.status === "active" ? "bg-gold/10 text-gold" : "bg-surface-3 text-text-muted"
                  }`}>{p.status}</span>
                </div>
                ...
                <div className="flex items-center gap-4 text-[11px] text-text-muted">
                  ...
                  <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-gold/60" />{p.taskCounts["in-progress"]} active</span>
                ...
                <div className="mt-4 pt-3 border-t border-border-subtle flex items-center justify-end">
                  <ArrowRight size={14} className="text-text-muted/30 group-hover:text-gold transition-colors" />
                </div>
              </Link>
            ))}
          </div>
